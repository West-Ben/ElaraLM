...
async def list_remote_models() -> List[str]:
    ...
    try:
        headers = (
            {"Authorization": f"Bearer {_llmstudio_key}"} if _llmstudio_key else {}
        )
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(
                _llmstudio_url.rstrip("/") + "/models", headers=headers
            )
            resp.raise_for_status()
            data = resp.json()
...
def select_model(
    source: str, name: str, url: str | None = None, key: str | None = None
) -> None:
    global _current_source, _current_model, _llmstudio_url, _llmstudio_key
...
async def generate(prompt: str) -> str:
    logger.info("Generating LLM response via %s", _current_source)
    if _current_source == "remote" and _llmstudio_url:
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                resp = await client.post(
                    url, json={"model": _current_model or "llama3", "prompt": prompt}
                )
                resp.raise_for_status()
                data = resp.json()
